...
MOTION_FILE_NAME = DIR + '/../../../data/noise_estimation/motion.txt'
...
class MotionNoiseRecorder:
    ...
    def callback(self, real, odom):
        ...
        if self.last_real_x != None:
            real_rot1 = atan2(real_y-self.last_real_y, real_x - self.last_real_x) - self.last_real_theta
            real_trans = (real_x-self.last_real_x)**2 + (real_y-self.last_real_y)**2
            # if real_trans < 1e-10:
                # return
            if real_rot1 <= -pi/2:
                real_trans *= -1
            ...
            odom_rot1 = atan2(odom_y-self.last_odom_y, odom_x - self.last_odom_x) - self.last_odom_theta
            odom_trans = (odom_x-self.last_odom_x)**2 + (odom_y-self.last_odom_y)**2
            if odom_rot1 <= -pi/2:
                odom_trans *= -1
                odom_rot1 += pi
            elif odom_rot1 > pi/2:
            ...
            tmp = str(real_rot1) + " " + str(real_trans) + " " + str(real_rot2) + " "
            tmp += str(odom_rot1) + " " + str(odom_trans) + " " + str(odom_rot2) + " "
            tmp += str(rot1_error) + " " + str(trans_error) + " " + str(rot2_error) + "\n"
